...
class RequestLogMiddleware:
    ...
    Штатный лог uvicorn показывает только адрес, с которого пришёл запрос, а
    за обратным прокси это всегда один и тот же локальный адрес — отличить
    вызов внешнего клиента от служебной проверки по нему нельзя. Именно на
    этом застрял разбор отказа Action: в логах были только строки
    "GET /health 200 OK" без признака отправителя, и вопрос "дошёл ли запрос
    от ChatGPT вообще" оставался без ответа. Поэтому пишем User-Agent и
    X-Forwarded-For — по ним отправитель опознаётся сразу.

    Именно этот лог и дал ответ: запросы от ChatGPT до сервиса не доходили,
    что и стало причиной переезда с Render (см. README).
...
def consensus_aastocks(symbol: str, authorization: Optional[str] = Header(default=None)):
    ...
    Не для автоматического опроса: по расписанию не вызывается ничем,
    обращения к сайту ограничены кэшем на тикер и суточным потолком (при
    исчерпании — 429). См. broker_consensus.py.
    """
    check_auth(authorization)
